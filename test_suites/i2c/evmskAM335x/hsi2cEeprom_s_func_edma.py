...
requires = ['evmskAM335x','i2c','uart','edma']
...
if (serial_load('hsi2c_eeprom_edma/Release/hsi2cEeprom_edma.bin') == 1): 
    platform.expect('Enter\s+the\s+bus\s+frequency',10)
    platform.send(i2c_frequency + '\r')
    platform.expect('Enter\s+an\s+address\s+in\s+EEPROM\s+starting\s+from\s+which',10)
    platform.send(address_to_read_from + '\r')
    platform.expect('Enter\s+the\s+number\s+of\s+data\s+bytes\s+to\s+be\s+read',10)
    platform.send(number_of_bytes_to_read + '\r')
    platform.expect('Data\s+read\s+from\s+EEPROM:',10)
