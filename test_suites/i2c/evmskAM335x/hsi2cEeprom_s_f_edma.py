...
if (serial_load('hsi2c_eeprom_edma/Release/hsi2cEeprom_edma.bin') == 1): 
    ...
    platform.expect('Enter\s+an\s+address\s+in\s+EEPROM\s+starting\s+from\s+which',timeout=100)
    platform.send(address_to_read_from + '\r')
    platform.expect('Enter\s+the\s+number\s+of\s+data\s+bytes\s+to\s+be\s+read',timeout=100)
